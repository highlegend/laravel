...
<a href="{{route('etudients.edit',$etudiant->id)}}"  class="btn btn-warning btn-sm">Edit</a>
<a href="{{route('etudients.show',$etudiant->id)}}"  class="btn btn-info btn-sm">Show</a>
<form action="{{route('etudients.destroy',$etudiant->id)}}" method="POST">
<button  type="submit" class="btn btn-danger btn-sm">Destory</button>
@csrf
@method('DELETE')
</form>
                </div>
